...
$cust_name = $db->query("SELECT name FROM customer ORDER BY name ASC");
$item_name = $db->query("SELECT name FROM item ORDER BY name ASC");
	...
	if(count($_POST)>0) {
		$itId = 0;
		$cusId = 0;
		$cust_id = $db->query("SELECT id FROM customer where name='" . $_POST["custName"] . "'");
		...
		$current_id = $db->query("INSERT into price (customer_id, item_id, unit_price, create_ts, update_ts, status) values (" .$cusId. "," .$itId. "," . $_POST["price"] . ", now(), now(), 4)"  );
	...
	{
       header("Location: cost.php#price");
	}
...
<td colspan="2"><label>Add Customer Price</label></td>
</tr>
<tr class="spaceUnder">
...
<tr>
<td colspan="2"><input type="submit" name="submit" value="Add Price" class="btnSubmit"></td>
</tr>
</table>
</div>
</form>
